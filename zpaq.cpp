/* zpaq.cpp v6.00 - Journaling incremental deduplicating archiving compressor
...
Allows -extract to clobber existing files. With -add, update files
even if the dates are identical.
...
Choose compression method for -add, from 1 (fastest) to 4 (best).
...
bool detailed=false;  // global: set by -detailed option
...
private:
  ...
  void read_args();  // read command line args, scan directories
...
void Jidac::usage() {
  printf(
  "zpaq 6.00 - Journaling incremental deduplicating archiving compressor\n"
  ...
  "  -method 1...4                 Compress faster...better (default 1)\n"
      ...
      while (d.findFilename(&filename)) {
        ...
        if (comment.s.size()>=4
          ...
          if (filename.s[17]=='c' && fdate>=19000000000000LL
            ...
            if (++fversion>version)  // roll back archive to here
              ...
              int64_t jmp=btol(s);
              if (detailed && command=="-list")
                printf("Version %d Jump from %1.0f to %1.0f by %1.0f\n",
                       fversion, double(in.tell()),
                       double(in.tell()+jmp), double(jmp));
              if (jmp<0) {
            ...
            if (isbreak) {
              fprintf(stderr, "Archive truncated to %1.0f\n",
                      double(block_offset));
              in.close();
              return block_offset;
            }
          ...
          // assuming N in ascending order.
            ...
            if (detailed && command=="-list")
              printf("Fragments %1.0f-%1.0f compressed to %d\n",
                     double(num), double(num+n-1), bsize);
            if (ht.size()!=num)
              fprintf(stderr,
            ...
            for (unsigned i=0; i<n; ++i) {
              ...
              if (detailed && command=="-list") {
                printf("%10.0f ", double(num+i));
                for (int j=0; j<20; ++j)
                  printf("%02x", ht[num+i].sha1[j]&255);
                printf(" %10d %14.0f\n", ht[num+i].usize,
                       double(ht[num+i].csize));
              }
            }
            data_offset+=bsize;
          }
          ...
          // Read into DT
            ...
            while (s<=end-9) {
              ...
              if (dtv.date && s<=end-8) {
                ...
                if (detailed && command=="-list" && na) printf("na=%d ", na);
                s+=na;  // skip attr
                if (s<=end-4) {
              ...
              if (detailed && command=="-list") {
                printf("%4d %s %s :", dtv.version,
                       datetostring(dtv.date).c_str(), fp);
                printList(dtv.ptr);
              }
            }
          }
        ...
        // Streaming format
          ...
          if (detailed && command=="-list") {
            printf("%10d ", size(ht));
            for (int j=0; j<20; ++j)
              printf("%02x", sha1result[j+1]&255);
            printf(" %10.0f %14.0f\n", double(usize),
                   segs ? double(-segs) : double(block_offset));
          }
...
// Read files and recursively scan directories in command line arguments
// (in files[]) and add to dt.
...
void compressBlock(StringBuffer* in, libzpaq::Writer* out,
  ...
  // Built in ZPAQL code for -faster, default, -better, -best
  const char* cfg[5]={
    "(no method 0)",
  ...
  int level=0;
  if (*method>='1' && *method<='4' && method[1]==0) {
    level=*method-'0';
    method=cfg[level];
  }
  ...
  else {  // (level 4) compress without preprocessing
    co.setInput(in);
    co.compress();
    co.endSegment(sha1ptr);
  }
...
void writeJidacHeader(libzpaq::Writer *out, int64_t date,
  ...
  StringBuffer is;
  is+=ltob(cdata);
  compressBlock(&is, out, "comp 0 0 0 0 0 hcomp post 0 end ", 0, 0,
...
void Jidac::add() {
  ...
  if (mode==JIDAC) {
    ...
    if (now==-1 || date<20120000000000LL || date>30000000000000LL) // 2012-3000?
  ...
  if (mode==TINY || mode==SOLID) {
    ...
    for (map<string, DT>::iterator p=dt.begin(); p!=dt.end(); ++p) {
      InputFile in;
      if (!in.open(p->first.c_str())) continue;
  ...
  for (map<string, DT>::iterator p=dt.begin(); p!=dt.end(); ++p) {
    if (p->second.edate && (force || p->second.dtv.size()==0
      ...
      else if (p->second.dtv.size()==0 || p->second.dtv.back().date==0)
      ...
      // In non-JIDAC mode, split large files into 16 MB fragments. Store
      ...
      // the hash to ht. Save fragment pointers in dt.
        ...
        do {
          ...
          if (c==EOF || (h<65536 && fragment>=4096) || fragment>=520192) {
            ++totalFragments;
            ...
            fragment=0;
            if (size(b)>15500000) write_fragments(job, b, block_start);
          }
        } while (c!=EOF);
      }
      in.close();
  ...
  for (unsigned i=htsize; i<=ht.size(); ++i) {
    if ((i==ht.size() || ht[i].csize>0) && is.size()>0) {  // write a block
      ...
      compressBlock(&is, &out,
        "comp 0 0 0 0 0 hcomp post 0 end", 0, 0,
  ...
  for (map<string, DT>::iterator p=dt.begin(); p!=dt.end();) {
    ...
    ++p;
    if (is.size()>1000 || (is.size()>0 && p==dt.end())) {
      compressBlock(&is, &out, 
...
#endif
      if (ok) printf("Created directory %s\n", path.c_str());
      path[i]='/';
    }
  }
}
...
ThreadReturn decompressThread(void* arg) {
  ...
  for (unsigned i=0; i<job.block.size(); ++i) {
        ...
        if (memcmp(sha1.result(), job.jd.ht[j].sha1, 20)) {
          for (int k=0; k<20; ++k) {
            if (job.jd.ht[j].sha1[k]) {  // all zeros is OK
              printf("Job %d: fragment %d size %d checksum failed\n",
    ...
    for (map<string,DT>::iterator p=job.jd.dt.begin();p!=job.jd.dt.end();++p){
      ...
      for (unsigned j=0; j<ptr.size(); offset+=job.jd.ht[ptr[j++]].usize) {
        ...
        if (!job.outf.isopen()) {
          ...
          if (dtr.written==0) {
            makepath(filename);
            printf("Job %d: extracting %s to %s\n", jobNumber, 
        ...
        // Write the fragment
        assert(offset>=0);
        job.outf.write(q, offset, job.jd.ht[ptr[j]].usize);
...
void Jidac::extract() {
  ...
  printf("Starting %d decompression jobs\n", threads);
  vector<ThreadID> tid(threads);
    ...
    for (int j=0; d.findFilename(&filename); ++j) {
      ...
      if (j<job.block[i].size) {
        ...
        if (sha1out[0] && memcmp(sha1out+1, sha1.result(), 20))
          printf("WARNING: %s checksum error\n", lastfile.c_str());
      }
      else
