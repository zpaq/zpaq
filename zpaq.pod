#   zpaq.pod - zpaq v6.60 man page source
#
#   Copyright
#
...
C<*> matches any string and C<?> matches any character.
...
C<zpaq> will do its best to extract as much data as it can if the archive
is damaged.
...
List I<files> within the archive, or list the entire archive contents
if I<files> is omitted. For each file or directory, show the version number,
last modified date, Windows attributes or Unix/Linux permissions, uncompressed
size, approximate compression ratio, and name of each file or directory.
...
required by the ZPAQ standard but are required by C<test> unless the
C<-fragile> option is given.
...
added and deleted and OK if dates are valid (1900 to 2999) and fragment
pointers are valid.
...
With C<extract -to> to another archive,
keep all versions, which has the effect of simply copying the archive.
This option is useful when combined with
C<-key> and C<-newkey> to encrypt, decrypt, or change the password
of an encrypted archive. I<files>, I<-only>, and I<-not> are ignored.
...
With C<add> do not save extra data used for error detection
...
With C<test>, allow the test to pass if block checksums or the redundant
fragment size lists are misssing.

=item -fragment I<N>

With C<add>, set the average dedup fragment size to 2^I<N> KB. I<N>
...
Set the encryption key for the output of C<extract -to> to a new archive.
If I<password>
...
With C<list>, C<-not =[=#/?]...> compares with external files
or another archive
(after renaming with C<-to>). The characters C<=>, C<#>, C</>, and C<?>
in any order after the first C<=> tell C<zpaq> not to list any
files that are equal, different, not found, or unknown respectively.
For example, C<-not ==/> means do not list any files that are equal
or not found.
Any files that are listed are shown with the corresponding character
to indicate the result of the comparison. The total number of each
type is reported regardless. Files are compared
by computing their SHA-1 hashes and comparing with the stored hashes.
The comparison result is unknown if fragment hashes or sizes are not
stored (e.g. if compressed with C<-method s... -fragile>).
Dates and attributes are not compared. Directories always match.
C<zpaq> returns an error status of 1 if not all files are equal.

When comparing two archives, files are compared by comparing their
fragment hashes. Identical files will be listed as unknown if they
are fragmented on different boundaries. This can happen if they are
compressed with different values of the C<-fragment> option, or
if one is streaming (C<-method s...>), or if both are streaming and
compressed with different block sizes (like C<-method s1...> and
C<-method s2...>).
...
For example:
    ...
    zpaq list archive -quiet 1k

lists all files that are 1000 bytes or larger.

C<add> and C<extract> will display progress,
...
C<-quiet d> is equivalent to C<-quiet 0> with additional detailed
output. C<-list> will show the fragment numbers for each file.

=item -since [[-]I<version>]

List only I<version> and later. If I<version> is negative
then list only the last -I<version> updates.
Default is 0 (all). For example:

    zpaq list archive -since -2
...
With C<extract>, specify the output. If one or more I<files>
is specified, then each file (or directory) is replaced with the
correponding I<name>. For example:

    zpaq extract archive file dir -to newfile newdir

extracts C<file> as C<newfile> and C<dir> as C<newdir>.

If I<files> is empty, then the entire archive contents is renamed
by appending the first I<name> as a prefix, inserting or removing
a C</> as necessary and removing the C<:> from a Windows drive letter.
For example:
...
If I<files> is empty and the first I<name> has a C<.zpaq> extension
then extract to a new archive, retaining only the latest version
of each file. This is faster than extracting and creating a new archive
because it works by copying only those data blocks referenced by at
least one file, then building a new index.
The new archive is not encrypted by default. If additional I<names>
are specified, then I<files> or the entire contents are renamed in
the new archive as above. For example:

    zpaq extract arc1 dir1 -to arc2.zpaq dir2

creates arc2.zpaq with dir1 saved as dir2.

    zpaq extract arc1 -to arc2.zpaq dir2

would save dir1 in arc1.zpaq as dir2/dir1 in arc2.zpaq.

With C<list>, show how files would be renamed. When comparing with C<-not =>,
C<-to> renames the external files to be compared, for example:

    zpaq list archive dir1 -to dir2 -not ==

lists the differences between internal C<dir1> and external C<dir2>.

When the first I<name> ends with C<.zpaq>, then compare two archives,
using the same renaming rules. For example:

    zpaq list arc1 dir1 -to arc2.zpaq dir2 -not =

compares dir1 in arc1.zpaq with dir2 in arc2.zpaq. Both archives may
be the same to compare files within the same archive. With C<extract>,
the archives must be different.
...
Returns 0 if successful or 1 in case of an error. Comparison
with C<list -not => returns 1 if any files differ or are not found.
...
I<zpaq> v6.60 is copyright (C) 2009-2014, Dell Inc. It is written by Matt
