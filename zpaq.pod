#   zpaq.pod - zpaq v7.11 man page source
#
#   Copyright
#
...
=item encrypt I<password>

With C<extract>, do not extract any files. Instead,
encrypt I<archive>C<.zpaq> to the first file in I<files> with I<password>.
If I<password> is C<"">, then decrypt. It is an error if the output file exists
unless C<-force> is used to overwrite it.
If I<archive> is multi-part, then the output is a concatenation as
a single file. If I<archive> has an index, then it will need to be
re-created from the output archive.

Encryption works with any files, not just archives.
Encryption is in the format described in C<-key> with a new random salt.
...
=item -repack I<new_archive>

With C<extract>, store the extracted files in I<new_archive> instead
of writing them individually to disk. It is an error if I<new_archive>
...
If the input is encrypted, then the output is encrypted with the
same password but a new salt and keystream.
