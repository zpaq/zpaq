#   Copyright
#
#      Copyright (C) 2011 Matt Mahoney <matmahoney@yahoo.com>
#
#   License
#
...
I<libzpaq> is a public domain C++ API for compressing or decompressing
...
preceded by an identifying 13 byte tag which does not
appear in that data.
...
=item C<extern void error(const char* msg);>
...
=item C<void Writer::put(int c) = 0;>
...
Returns the length of the string whose hash is represented.
The largest possible value returned is
2^61 - 1 = 2305843009213693951.0. The initial value is 0.0.
...
=item C<void postProcess(const char* pcomp = 0);>
...
If C<pcomp> is not 0, then the first two bytes must encode
...
bytes and the string as passed. For example,
C<pcomp("\x02\x00\x05\x08");>
...
Decode n byte or until the end of segment, whichever comes
    ...
    void decompress() {
        ...
        while (d.findBlock()) {
            while (d.findFilename()) {
                ...
                if (sha1string[0]==1
                         && memcmp(sha1string+1, sha1.result(), 20))
...
These settings will work on most machines but are not
guaranteed by the C/C++ standard.
In particular, C<int> could be 16 bits.
C<U32> really ought to be defined as C<uint32_t>, but some
...
without -DNDEBUG, element accesses are bounds checked. The maximum
size is 2^30 - 1 bytes (under 1 GB).

    template <typename T>
    class Array {
    public:
        Array(int sz=0, int ex=0);
        void resize(int sz, int ex=0);
        int size() const;
        T& operator[](int i);
        T& operator()(int i);
    };
...
=item C<Array(int sz=0, int ex=0);>
...
allocated. If the requested size exceeds 1^30 - 1 (for
example, C<Array<int> a(1, 100);>) then it calls C<error("Array too big")>.
If allocation fails, then it calls C<error("Out of memory")>.

=item C<resize(int sz=0, int ex=0);>

Changes the size of the Array as with creation and zeros the memory.

=item C<int size() const;>

Returns the number of elements.

=item C<T& operator[](int i);>
...
output->put(A) for each output byte A. If 0, then
...
SHA-1 checksums. It calls sha1->put(A) for each output byte A.
...
I<libzpaqo> or I<zpaqopt> generated by zpaq(1).
