/* unzp 1.00 - Parallel, optimizing ZPAQ archive extractor
...
unzp is a parallel, optimizing ZPAQ archive extractor. It will
extract any archive conforming to the ZPAQ level 1 specification
described at http://mattmahoney.net/dc/zpaq.html
Thus, it will decompress any archive created by any of the compressors
described on that page, such as zpaq, pzpaq, zpipe, and zpsfx.

unzp runs faster than the reference decoder, unzpaq, because it will
decompress ZPAQ blocks in parallel in separate threads. Also, if enabled,
it will translate decoding algorithms from the ZPAQ block headers into C++,
compile them with an external compiler, link them to itself and
run the optimized version about twice as fast.

Usage: unzp [-options] command files...

Each file should be a ZPAQ archive. Commands:

  c = concatenate to standard output, ignoring saved file names.
...
A ZPAQ archive can be created with or without saved file names.
If no file names are saved, then e and x have the same effect as d.
Files that have neither saved names nor a .zpaq extension are
decompressed to file.out. x will create subdirectories as needed.

Options:

  -tN = use N threads (default = number of available processors).
  -f  = force overwrite of existing files.
  -r  = remove input files after extraction.
  -k  = keep temporary unzp*.cpp optimization file.
  -v  = verbose

A ZPAQ archive can be divided into independent blocks that can
be decompressed in parallel by separate threads. The l command
will show the contents of each block, which can be a file, a
part of a file, or more than one file. It will also show how
much memory is required to decompress it. By default, unzp will
run one thread per processor to decompress as many blocks as
possible at one time. The memory required is the sum of the
requirements of each block running at the same time.

The -t option can be used to reduce the number of threads and
therefore reduce the required memory at the expense of speed.
Increasing -t above the number of processors will not increase
speed but will increase the memory required. Increasing -t above
the number of input blocks will have no effect.

The default number of threads is set to the environment
variable NUMBER_OF_PROCESSORS in Windows, or from /proc/cpuinfo
in Linux (by counting lines of the form "processor : %d").
...
filenames have the form "unzptmp{pid}_{N}" where {pid} is the
process ID and N is a number 1 or more. They are normally
deleted after use.

If an external C++ compiler is available, then unzp can be
...
will run faster if JIT is enabled when unzp.cpp is compiled. If
enabled, then unzp will generate a temporary file containing
C++ code (unzptmp{pid}_0.cpp), and attempt to compile, link,
...
This program compiles for Windows and Unix variants (including Linux).
To compile, you need libzpaq.cpp and libzpaq.h from
libzpaq from http://mattmahoney.net/dc/zpaq.html
To compile without JIT optimization:

  g++ -O3 -DNDEBUG unzp.cpp libzpaq.cpp -o unzp

-DNDEBUG turns off run time debugging checks in libzpaq. -DNOOPT
includes a default set of optimized code. Without this option, the
program would have to be linked with optimized code either
from libzpaqo.cpp (for models fast, mid, max) or from a
temporary file generated by "unzp -k" or "zpaq ok".

Additional recommended optimizations besides -O3 are
-s -march=native -fomit-frame-pointer
For distribution on x86-32 machines, you can use -march=pentiumpro for
compatibility with older machines and no significant performance loss.

To enable JIT, the following files must be installed at some fixed location
and unzp must be compiled with an option (-DOPT) telling it where they
can be found:

  unzp.o       (compiled without -DOPT or -DNOOPT)
  libzpaq.o    (compiled from libzpaq.cpp)
  libzpaq.h    (from libzpaq)

The option -DOPT should be set to a command string to compile a
file %1.cpp to %1.exe, where unzp will substitute %1 with the name
of a temporary file. This file will be linked with the two .o
(or .obj) files and #include the above header file. For example:

  g++ -O3 -c -DNDEBUG libzpaq.cpp
  g++ -O3 unzp.cpp -o unzp -DOPT="g++ -O3 -DNDEBUG %1.cpp unzp.o libzpaq.o -o %1.exe"
  g++ -O3 -c -DNOOPT unzp.o

When run, unzp will expect to find the above 3 files in the current
directory. If they are elsewhere, then the -DOPT string should specify
their exact location and include a -I option to find libzpaq.h.
Note that unzp.exe is not made from the same unzp.o described in -DOPT.
...
with JIT enabled as follows:
  ...
  -DOPT="\"g++ -O3 -DNDEBUG -Ic:\\bin\\zpaq c:\\bin\\zpaq\\unzp.o c:\\bin\\zpaq\\libzpaq.o %1.cpp -o %1.exe\""

In Linux, you also need to compile unzp.cpp with -lpthread. Also, -Dunix
...
void usage() {
  fprintf(stderr,
  "unzp v1.00 - Parallel ZPAQ archive extractor\n"
  "(C) 2011, Dell Inc. Written by Matt Mahoney\n"
  "This is free software under GPL v3. http://www.gnu.org/copyleft/gpl.html\n"
  "\n"
  "Usage: unzp [-options] command files...\n"
  "Commands:\n"
  "  c   Decompress and concatenate to standard output\n"
  "  d   Decompress each file.zpaq to file\n"
  "  e   Extract to current directory using saved names\n"
  "  x   Extract to original directories using saved paths\n"
  "  l   List contents\n"
  "Options:\n"
  "  -f  Force overwrite of existing files\n"
  "  -k  Keep temporary JIT source code\n"
  "  -r  Remove input files after decompression\n"
...
#endif
#ifdef OPT
  fprintf(stderr, "JIT optimization enabled with:\n  %s\n", OPT);
#else
...
// File for libzpaq decompression
...
int main(int argc, char** argv) {
  ...
  // Process arguments: unzp [-options] command files...
  if (argc<3) usage();
  topt=numberOfProcessors();
  int filelist=0;  // index of first file in argv
  for (int i=1; i<argc; ++i) {
    if (command==' ' && argv[i][0]=='-') {
      switch(argv[i][1]) {
      ...
      command=argv[i][0];
      if ((command!='c' && command!='d' && command!='e'
          && command!='x' && command!='l') || argv[i][1])
        usage();
    }
    else if (!filelist)
      filelist=i;
  ...
  // Schedule decompression for commands c, d, e, x.
  ...
  if (command=='c' || command=='d' || command=='e' || command=='x') {
#ifdef OPT
