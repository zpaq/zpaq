...
(C) 2009, Ocarina Networks, Inc.
    ...
    The GPL license only applies to this program (zpaqsfx.cpp) and
    corresponding executable stubs produced by compiling this program
    (for example, zpaqsfx.exe). Executable self extracting archives created
    by appending ZPAQ archives to these stubs (as described below) may
    be distributed without restriction.

This program is a stub for creating self extracting files using ZPAQ.
To create an archive, copy the executable and append a ZPAQ archive
to it. For example:

  copy zpaqsfx.exe archive.exe
  zpaq a archive.exe files...

When archive.exe is run with no arguments, it will list the contents
of the archive. When run with the argument "x", it will extract
the files. When run with "x" followed by a list of filenames, it
will rename the files as they are extracted in the same order that
they are listed and stop after the last given file name. For example:

  archive.exe               (list contents and instructions to extract)
  archive.exe x             (extract all files)
  archive.exe file1 file2   (extract and rename first 2 files)

archive.exe must be run in the current directory or by specifying
the path to it when run. It will not work if it is moved
to a directory listed in the user's PATH variable and run as a command
without a path. For example:

  archive.exe x             (OK)
  move archive.exe \bin     (assume \bin in PATH)
  archive.exe x             (fails, can't find archive.exe)
  \bin\archive.exe x        (OK)

You may omit the .exe extension:

  archive x                 (OK)

The self extractor will not create directories. If you wish to
distribute a folder full of files, then create an archive containing
only the bare filenames (using "zpaq a" in v1.03 and higher) and
instruct the user to create any needed directories, e.g.

  mkdir tmp
  cd tmp
  ..\archive x

To create zpaqsfx.exe, compile this file, then append the file
zpaqsfx.tag. This file contains 16 random bytes which the program
will search for to find the start of the archive. For example:

  g++ -DNDEBUG -O2 -s -fomit-frame-pointer -march=pentiumpro zpaqsfx.cpp
  upx a.exe
  copy/b a.exe+zpaqsfx.tag zpaqsfx.exe

Use -DNDEBUG to turn off run time checks, which makes the program
smaller and faster. Other compilers or executable packers may also be
used. The choice of compiler options is not critical.
...
// Open filename, and if that doesn't work, filename.exe, and
// if that doesn't work, give up with an errror.
FILE *open_self(const char* filename) {
  ...
  in=fopen(newname, "rb");
  if (in) return in;
  printf("Error: can't find %s or %s\n", filename, newname);
  exit(1);
}
  ...
  while (find_start(in)) {
    ...
    while ((c=getc(in))==1) {
      ...
      if (filename[0] || !out) {
        ...
        // If the user gave an output file starting at argv[2], use it instead.
        // If the user doesn't name all the files, then stop after the last
        // named file.
        if (argc>2) {
          if (filecount+2 < argc) {
            out=fopen(argv[filecount+2], "wb");
            if (!out) {
              perror(argv[filecount+2]);
              goto end;
            }
            else
              printf("Decompressing %s -> %s ... ", filename, argv[filecount+2]);
          }
          else {
...
void list(int argc, char** argv) {
  ...
  printf("\n"
    ...
    "%1.3f MB memory required to extract.\n",
     argv[0], argv[0], max_memory/1000000.0);
}
