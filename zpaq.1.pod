...
=head1 SYNOPSIS
  ...
  To compress:     zpaq [-bfhimnpqstv] {a|c} <archive> <files>...
  To extract:      zpaq [-finpstv] x <archive> [<files>...]
...
Tracing displays instructions and virtual registers during execution and
...
then the entire archive is extracted. In this case,
...
Force overwrite of output files when extracting with B<x>.
...
I<zpaq> v4.00 is copyright (C) 2011, Dell Inc. It is written by Matt
