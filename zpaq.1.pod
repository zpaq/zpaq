...
For all commands operating on archives (B<a>, B<d>, B<l>,
...
Compare the internal file named I<file> in I<archive>C<.zpaq>
to the file I<output>. If I<output> does not exist then it is
created. If it exists and they are different, 
then I<output> is overwritten. If the archive
does not contain any file named exactly I<file> then it is
an error and I<output> is not created or modified.
Comparison is done as as with B<a>.

If I<file> is omitted, then I<output> will be a concatenation
of the entire contents of the archive. The operation is skipped
if the contents of I<output> would not be changed.

If I<output> is also omitted, then each file in the archive
will be compared with the external file of the same name.
If the external file does not exist, then it will be created.
Otherwise the existing file is I<not> modified. Instead
the results of the comparison is shown. Each output file is
listed preceded by the character "=" if the file is identical, "#"
if different, or ">" if the external file is created.

Directories are created
as needed. A saved path may use either forward slashes (/)
or backslashes (\) as directory separators. During extraction,
"/" is converted to "\" in Windows, or "\" to "/" in Linux.
If the first segment of the archive is not named, then the
name is taken by dropping C<.zpaq> from the archive name.
...
Furthermore, B<-bs> does not save original file sizes, checksums,
or header locator tags, which saves a little extra space.
That information is only needed for incremental update or
recovery of subsequent blocks from corrupt archives,
which would not be possible anyway.
...
I<zpaq> v4.01 is copyright (C) 2011, Dell Inc. It is written by Matt
